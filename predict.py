...
# Load model
MODEL_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), 'rf_model_pickle.pkl'))
...
def predict():
    ...
    try:
        ...
        if model is not None:
            ...
            prediction, risk_score, confidence = fallback_prediction(data, bmi, cholesterol_cat, glucose_cat)
...
def fallback_prediction(data, bmi, chol_cat, gluc_cat):
    """Fallback prediction when model is not available
    Logic based on simplified Framingham Risk Score concepts
    """
    print('Fallback prediction logic ussssssseeeeeeeeee::::::::')
    risk_score = 0
    ...
    if ap_hi > 160: risk_score += 25
    elif ap_hi > 140: risk_score += 15
    elif ap_hi > 130: risk_score += 10
    
    # Cholesterol (Encoded)
    if chol_cat == 3: risk_score += 25
    elif chol_cat == 2: risk_score += 15
    ...
    # Glucose (Encoded)
    if gluc_cat == 3: risk_score += 15
    elif gluc_cat == 2: risk_score += 10
...
if __name__ == '__main__':
    ...
    print("WARNING: Use the included fallback logic for testing if model files are missing.")
    print("="*60)
